...
var initializeHeartbeat = (function() {
	// Wait at least 8 hours between server contacts
	var nIntervalLength = 8 * 60 * 60 * 1000;

	// localStorage key to store last run time
	var sLastRunKey = 'TNCMSUserHeartbeatTime';
	var sHeartbeatLock = 'TNCMSUserHeartbeatLock';
	...
	function heartbeat() {
		var nNow = Date.now(),
			...
			nLock = localStorage.getItem(sHeartbeatLock),
			sScreenName = getScreenName();
		
		if (nLock) { // If there's a lock
			if (nLock >= (nNow - (5 * 60 * 1000))) { // If the lock is less than 5 minutes old, skip
				return;
			}
			else { // Otherwise, blow up the lock
				localStorage.removeItem(sHeartbeatLock);
			}
		}
		...
		// We've determined to make a request - place a lock
		localStorage.setItem(sHeartbeatLock, nNow);
		...
		oReq.addEventListener('load', function() {
			if (oReq.status >= 400) {
				// The heartbeat did not fire correctly, assume a failure
				console.error('Heartbeat error, will try again later');
				return;
			}
...
window.TNCMS.User = {
	...
	/* }}} */
	// {{{ hasWebauthnCredential( void ) : bool

	/**
	 * Check if the current client has at least one Webauthn credential registered for reauthentication.
	 *
	 * @return bool
	 *		Returns true if the client has at least one registered Webauthn credential.
	 */
	hasWebauthnCredential: function() {
		return !! getCookieValueByName('tncms-webauthn-available');
	},

	// }}}
	// {{{ hasFullAccess( void ) : bool

	/**
	 * Determine whether the current user has full logged-in access.
	 *
	 * @return bool
	 */
	hasFullAccess: function() {
		var sJWT = getCookieValueByName('tncms-user');
		if ( ! sJWT )  return false;
